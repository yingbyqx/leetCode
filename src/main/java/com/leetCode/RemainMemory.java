...
public class RemainMemory {
        ...
        while (true){
            if(memory1 >= memory2){
                if(memory1 >= time){
                    ...
                    return dealResult(memory1, memory2, remain, time);
                }
            }else {
                if(memory2 >= time){
                    memory2 -= time;
    ...
    private int[] dealResult(int memory1, int memory2, int[] remain, int time) {
        remain[0] = time;
        remain[1] = memory1;
        remain[2] = memory2;
        return remain;
    ...
    public static void main(String[] args) {
        RemainMemory memory = new RemainMemory();
        System.out.println(JSON.toJSONString(memory.memLeak(8, 11)));
    }
}
